...
class Stack:
    ...
    ctrl_list_level_1 = ['T']
    ctrl_list_level_2 = ['TG']
    # ctrl_list_level_2 = ['R2','R3','R4','R5','R6','R71','R72','R73','R74','R75','R76','R77','R78','R79','R8','R9']
    # ctrl_list_level_3 = ['B91', 'B92', 'B93', 'B71', 'F23', 'F239']
    ctrl_list_level_3 = [  'TG1']
    ctrl_list_level_4 = ['TG17']
    ctrl_set = set()
    ...
    def continue_detection_once_more(self, driver, i, m, q):
        ...
        if final_len_validation == 1 or final_len_validation == 0:
            ...
            if len(m_next_validation) == 1:
                final_str_validation = m_next_validation[0] + "|" + '---'
            else:
    ...
    def next_level_detection(self, i, m, len0, q, driver, next_str):
        ...
        for i in range(1, len5 + 1):
            ...
            if len5 == 1:
                self.ctrl_set.add(driver.current_url)
                if len(m_next) == 1:
                    l_str = m_next[0] + "|" + '---'
                else:
                ...
                if q.peek() != l_str:
                    q.push(l_str)
                self.continue_detection_once_more(driver, 1, m, q)
                if driver.current_url in self.ctrl_set:
                    print("this level has only one element and has been visited")
                    q.pop()
                    driver.back()
                # if i == len5:
                #     q.pop()
    ...
    def run(self):
        ...
        # driver = webdriver.Chrome(r"D:\Test\ChromeDriver\chromedriver.exe")  # 用chrome浏览器打开
        ...
        for i in range(1, len1 + 1):
            ...
            for i in range(1, len2 + 1):
                if m2[0 + (i - 1) * 2] not in self.ctrl_list_level_2:
                    continue
                if len2 > 1:
                    level2_str = m2[0 + (i - 1) * 2] + '|' + m2[(i - 1) * 2 + 1] + ':'
                    print(level2_str)
                    self.res_file.write(level2_str + '\n')
                    q.push(m2[0 + (i - 1) * 2] + '|' + m2[(i - 1) * 2 + 1])
                    m3 = self.click_and_find_elements(q, driver, i)
                    len3 = (int(len(m3) / 2))
                    pop3 = q.peek()
                    for i in range(1, len3 + 1):
                        if m3[0 + (i - 1) * 2] not in self.ctrl_list_level_3:
                            continue
                        if len3 > 1:
                            level4_str = m3[0 + (i - 1) * 2] + '|' + m3[(i - 1) * 2 + 1] + ':'
                            print(level4_str)
                            self.res_file.write(level4_str + '\n')
                            q.push(m3[0 + (i - 1) * 2] + '|' + m3[(i - 1) * 2 + 1])
                            m4 = self.click_and_find_elements(q, driver, i)
                            len4 = (int(len(m4) / 2))
                            for i in range(1, len4 + 1):
                                if m4[0 + (i - 1) * 2] not in self.ctrl_list_level_4:
                                    continue
                                self.next_level_detection(i, m3, len3, q, driver, "")
                                self.res_file.flush()
                                if i == len4:
                                    q.pop()
                                    driver.back()
